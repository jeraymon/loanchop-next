...
- **Redirects:** Next.js `redirects()` is dead code with `output: 'export'`. Redirects are managed in the **Amplify Console** → App settings → Rewrites and redirects. The Console is authoritative; a version-controlled mirror lives at `amplify-redirects.json` at the site root. **Do NOT put redirect rules in `customHttp.yml`** — Amplify silently ignores `customRules:` blocks there. `customHttp.yml` is headers-only.
...
- **SSG + AWS Amplify**: `output: 'export'` means Next.js `redirects()` is dead code. Redirects are managed in the **Amplify Console** (App settings → Rewrites and redirects) and mirrored to `amplify-redirects.json` in the repo for version control. `customHttp.yml` is for custom HTTP headers ONLY — Amplify silently ignores any `customRules:` block there. Use wildcard rules per legacy directory (e.g., `/phpforce/<*>` → `/force/`) where possible; enumerate specific `.php` files when one directory contains multiple distinct target calculators.
